...
#ce
  ...
  If NOT FileExists($UserHome&"\VirtualBox.xml") Then
    ...
    Local $XmlContent = '<?xml version="1.0"?>' & @CRLF & _
        '<!--' & @CRLF & _
        '** DO NOT EDIT THIS FILE.' & @CRLF & _
        '** If you make changes to this file while any VirtualBox related application' & @CRLF & _
        '** is running, your changes will be overwritten later, without taking effect.' & @CRLF & _
        '** Use VBoxManage or the VirtualBox Manager GUI to make changes.' & @CRLF & _
        '-->' & @CRLF & _
        '<VirtualBox xmlns="http://virtualbox.org" version="1.12-windows">' & @CRLF & _
        '  <Global>' & @CRLF & _
        '    <ExtraData>' & @CRLF & _
        '      <ExtraDataItem name="GUI/LastItemSelected" value="n=GLOBAL"/>' & @CRLF & _
        '      <ExtraDataItem name="GUI/LastWindowPosition" value="512,264,640,488"/>' & @CRLF & _
        '      <ExtraDataItem name="GUI/Tools/LastItemsSelected" value="Welcome,Details"/>' & @CRLF & _
        '    </ExtraData>' & @CRLF & _
        '    <MachineRegistry/>' & @CRLF & _
        '    <NetserviceRegistry>' & @CRLF & _
        '      <DHCPServers/>' & @CRLF & _
        '    </NetserviceRegistry>' & @CRLF & _
        '    <Updates enabled="false">' & @CRLF & _
        '      <Host enabled="false" channel="1" checkFreqSec="86400" repoUrl="https://virtualbox.org" checkCount="0"/>' & @CRLF & _
        '    </Updates>' & @CRLF & _
        '    <SystemProperties defaultMachineFolder="' & $MachineFolder & '" defaultHardDiskFormat="VDI" VRDEAuthLibrary="VBoxAuth" webServiceAuthLibrary="VBoxAuth" LogHistoryCount="3" proxyMode="0" exclusiveHwVirt="false"/>' & @CRLF & _
        '    <USBDeviceFilters/>' & @CRLF & _
        '  </Global>' & @CRLF & _
        '</VirtualBox>'

	$file = FileOpen($UserHome&"\VirtualBox.xml", 2 + 8)
